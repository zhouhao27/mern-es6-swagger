...
let globalNewsDoc = {}
const NEWYORKTIMES_CATEGORIES = ["home",
  "world",
  "national",
  "business", 
  "technology"
...
// Can't reuse db connection in child process
// TODO: reuse the code
let url = process.env.MONGODB_CONNECT_URL
let db = {}
if (process.env.NODE_ENV !== 'production') 
  url = process.env.LOCAL_MONGODB_CONNECT_URL
...
client.connect(err => {
  ...
  console.log(`Fork is connected to MongoDB server at ${url}`)
})
...
process.on('message', (m) => {
  if (m.msg) {
    if (m.msg == Constants.MSG_FRESH_STORIES) {
...
//
var count = 0;
newsPullBackgroundTimer = setInterval(() => {  
  ...
  async.timesSeries(NEWYORKTIMES_CATEGORIES.length, (n, next) => {
    setTimeout(() => {
    ...
    if (err) {
      ...
      db.collection.findOne({ _id: process.env.GLOBAL_STORIES_ID }, (err, gDoc) => {
        if (err) {
          ...
          async.eachSeries(allNews, (story, innercallback) => {
...
//
staleStoryDeleteBackgroundTimer = setInterval(() => {
...
}, Constants.STALESTORYDELETE_TIMER);